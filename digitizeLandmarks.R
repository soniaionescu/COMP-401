...
## before running this, you must have the packages "jpeg" and "tidyverse" installed and loaded, and type "require(tidyr)".
...
digitize_landmark <- function(filename){
    ...
    fishPhotos <- read.csv(file=filename)
    i=1 ## start at the first entry
    ...
    df <- data.frame(ftLipx, ftLipy, bMouthx, bMouthy, fbLipx, fbLipy, bGillSlitx, bGillSlity, bGillx, bGilly, tGillSlitx, tGillSlity, fPecx, fPecy, bPecx, bPecy, fPelx, fPely, bPelx, bPely, fAnx, fAny, bAnx, bAny, bCaudPenx, bCaudPeny, lCaudx, lCaudy, blLobeCaudx, blLobeCaudy, cCaudx, cCaudy, buLobeCaudx, buLobeCaudy, tCaudx, tCaudy, tCaudPenx, tCaudPeny, bSDx, bSDy, bmSDx, bmSDy, mSDx, mSDy, uSDx, uSDy, fSDx, fSDy, bSpDx, bSpDy, lSpDx, lSpDy, uSpDx, uSpDy, mSpDx, mSpDy, fSpDx, fSpDy, c30x, c30y, tHeadx, tHeady, lEyex, lEyey, bEyex, bEyey, rEyex, rEyey, tEyex, tEyey, m35x, m35y, nx, ny, rHeadx, rHeady)
    while(i <= length(fishPhotos)){
        plot_jpeg(fishPhotos[i,1])
        ji <- locator(39, type = "l") ## first, locate all the points on the fish (in the correct order)
